...
export const getCompanyReviews = async (req, res) => {
  try {
    ...
    let company = await employeeProfileModel.findOne({ authId: companyId });
    if (!company) {
      company = await employeeProfileModel.findById(companyId);
    }
    ...
    const totalReviews = await companyReviewModel.countDocuments({
