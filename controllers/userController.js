...
function autoUpdateReviewStatus(profile) {
  if (profile.profileScore === 100 && profile.reviewStatus === "pending") {
    profile.reviewStatus = "underReview";
  }
}
...
export const fetchApplicants = async (req, res) => {
  try {
    const userId = req.user._id; // auth id
    ...
    const applications = await applicationModel
      ...
      .populate(
        "job",
        "title description location category jobType salaryType fixedSalary minSalary maxSalary",
      );
...
export const followUnfollowAccount = async (req, res) => {
  ...
  try {
    ...
    if (!isFollowing) {
      ...
      // UNFOLLOW
      followedProfile.followersAccounts =
        followedProfile.followersAccounts.filter(
          (id) => id.toString() !== followerProfileId,
        );

      followerProfile.followedAccounts =
        followerProfile.followedAccounts.filter(
...
export const getApplicantDashboardStats = async (req, res) => {
  ...
  try {
    ...
    const reviewsCount = await companyReviewModel.countDocuments({
      reviewerId: user._id,
    });
...
export const uploadProjectImages = async (req, res) => {
  ...
  if (projectIdx === undefined) {
    return res
      .status(400)
      .json({ success: false, message: "Project index is required" });
  }
  if (!req.files || req.files.length === 0) {
    return res
      .status(400)
  ...
  try {
    ...
    if (Number.isNaN(idx) || idx < 0 || idx >= user.projects.length) {
      return res
        .status(400)
        .json({ success: false, message: "Invalid project index" });
    }
