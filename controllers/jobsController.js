...
export const addJob = async (req, res) => {
  ...
  try {
    ...
    if (!employeeProfile.sentJobs.includes(job._id)) {
      employeeProfile.sentJobs.push(job._id);
    }
...
export const getCompanyJobs = async (req, res) => {
  ...
  try {
    const companyJobs = await jobsModel.find({
      slug,
      isActive: true,
      approved: "approved",
    });
...
export const getSavedJobs = async (req, res) => {
  ...
  try {
    const user = await userProfileModel.findOne({ authId: userId });
    const savedJobs = await jobsModel
      .find({ _id: { $in: user.savedJobs }, isActive: true })
      .populate("postedBy");
...
export const updateJobStatus = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: "Job Status Updated" });
  } catch (error) {
    return res.status(500).json({ success: false, message: error.message });
  }
};
...
export const getCompanyJobsById = async (req, res) => {
  ...
  try {
    const companyJobs = await jobsModel
      .find({ postedBy: id })
      .populate("postedBy");
...
export const updateJob = async (req, res) => {
  ...
  try {
    ...
    // Verify ownership
    if (job.postedBy.toString() !== employeeProfile._id.toString()) {
