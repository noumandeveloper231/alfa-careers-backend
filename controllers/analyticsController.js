...
export const weeklyAnalytic = async (req, res) => {
  try {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const count = await employeeProfileModel.countDocuments({
        createdAt: { $gte: end, $lt: start },
      });
...
export const getDashboardStats = async (req, res) => {
  try {
    ...
    const employeesThisMonth = await employeeProfileModel.countDocuments({
      createdAt: { $gte: startOfMonth },
    });
    ...
    const employeesLastMonth = await employeeProfileModel.countDocuments({
      createdAt: { $gte: startOfLastMonth, $lte: endOfLastMonth },
    });
    ...
    const stats = {
      ...
      growth: {
        ...
        employees: calculateGrowth(employeesThisMonth, employeesLastMonth),
...
export const applicationAnalytics = async (req, res) => {
  try {
    ...
    const totalApplications = await applicationModel.countDocuments();
    const hiredApplications = await applicationModel.countDocuments({
      status: "hired",
    });
    const successRate =
      totalApplications > 0
...
export const userAnalytics = async (req, res) => {
  try {
    ...
    const activeUsers = await applicationModel
      .distinct("applicant")
      .then((ids) => ids.length);
    const totalUsers = await userProfileModel.countDocuments();
    ...
    // Recent users
    const recentUsers = await userProfileModel
      .find()
...
export const systemAnalytics = async (req, res) => {
  try {
    ...
    const collections = [
      ...
      {
        name: "Employees",
        count: await employeeProfileModel.countDocuments(),
      },
    ...
    const recentActivity = {
      newUsers: await userProfileModel.countDocuments({
        createdAt: { $gte: yesterday },
      }),
      newJobs: await jobsModel.countDocuments({
        createdAt: { $gte: yesterday },
      }),
      newApplications: await applicationModel.countDocuments({
        createdAt: { $gte: yesterday },
      }),
      newBlogs: await blogModel.countDocuments({
        createdAt: { $gte: yesterday },
      }),
    };
